const FORWARD_S_BOX: [u8; 256] = [
    ,
];

const INVERSE_S_BOX: [u8; 256] = [
    ,
];

const ROUND_CONSTANTS: [u8; 11] = [
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36,
];

fn add_round_key(state: &mut [u8], key_schedule: &[u8], round: usize) {
    for i in 0..16 {
        state[i] ^= key_schedule[i + round * 16];
    }
}

pub fn decrypt_block(block: &[u8], key: &[u8]) -> Result<Vec<u8>, String> {
    if block.len() != 16 {
        return Err(format!(
            "Invalid block size; expected 16, got {}",
            block.len()
        ));
    }

    let rounds = match key.len() {
        16 => 10,
        24 => 12,
        32 => 14,
        _ => return Err(format!("Invalid key length: {}", key.len())),
    };
    let key_schedule = get_key_schedule(&key)?;
    let mut state = block.to_vec();

    add_round_key(&mut state, &key_schedule, rounds);

    // Starts at one, so only applies `rounds - 1` rounds.
    for round in (1..rounds).rev() {
        inverse_shift_rows(&mut state);
        sub_bytes(&mut state, &INVERSE_S_BOX);
        add_round_key(&mut state, &key_schedule, round);
        inverse_mix_columns(&mut state);
    }

    inverse_shift_rows(&mut state);
    sub_bytes(&mut state, &INVERSE_S_BOX);
    add_round_key(&mut state, &key_schedule, 0);

    Ok(state)
}

pub fn encrypt_block(block: &[u8], key: &[u8]) -> Result<Vec<u8>, String> {
    if block.len() != 16 {
        return Err(format!(
            "Invalid block size; expected 16, got {}",
            block.len()
        ));
    }

    let rounds = match key.len() {
        16 => 10,
        24 => 12,
        32 => 14,
        _ => return Err(format!("Invalid key length: {}", key.len())),
    };
    let key_schedule = get_key_schedule(&key)?;
    let mut state = block.to_vec();

    add_round_key(&mut state, &key_schedule, 0);

    // Starts at one, so only applies `rounds - 1` rounds.
    for round in 1..rounds {
        sub_bytes(&mut state, &FORWARD_S_BOX);
        shift_rows(&mut state);
        mix_columns(&mut state);
        add_round_key(&mut state, &key_schedule, round);
    }

    sub_bytes(&mut state, &FORWARD_S_BOX);
    shift_rows(&mut state);
    add_round_key(&mut state, &key_schedule, rounds);

    Ok(state)
}

fn get_key_schedule(key: &[u8]) -> Result<Vec<u8>, String> {
    let key_length = key.len();
    let size = match key_length {
        16 => 176,
        24 => 208,
        32 => 240,
        _ => return Err(format!("Invalid key length: {}", key_length)),
    };
    let mut expanded_key: Vec<u8> = Vec::with_capacity(size);

    for i in (0..size).step_by(4) {
        if i < key_length {
            expanded_key.push(key[i]);
            expanded_key.push(key[i + 1]);
            expanded_key.push(key[i + 2]);
            expanded_key.push(key[i + 3]);
        } else if i % key_length == 0 {
            // Note that this branch implements the rotation operation when
            // lookup up indices in `expanded_key`: 0 => -3, 1 => -2, 2 => -1,
            // and 3 => -4.
            expanded_key.push(
                expanded_key[i - key_length]
                    ^ FORWARD_S_BOX[expanded_key[i - 3] as usize]
                    ^ ROUND_CONSTANTS[i / key_length],
            );
            expanded_key.push(
                expanded_key[i + 1 - key_length] ^ FORWARD_S_BOX[expanded_key[i - 2] as usize],
            );
            expanded_key.push(
                expanded_key[i + 2 - key_length] ^ FORWARD_S_BOX[expanded_key[i - 1] as usize],
            );
            expanded_key.push(
                expanded_key[i + 3 - key_length] ^ FORWARD_S_BOX[expanded_key[i - 4] as usize],
            );
        } else if key_length > 24 && i % key_length == 16 {
            expanded_key
                .push(expanded_key[i - key_length] ^ FORWARD_S_BOX[expanded_key[i - 4] as usize]);
            expanded_key.push(
                expanded_key[i + 1 - key_length] ^ FORWARD_S_BOX[expanded_key[i - 3] as usize],
            );
            expanded_key.push(
                expanded_key[i + 2 - key_length] ^ FORWARD_S_BOX[expanded_key[i - 2] as usize],
            );
            expanded_key.push(
                expanded_key[i + 3 - key_length] ^ FORWARD_S_BOX[expanded_key[i - 1] as usize],
            );
        } else {
            expanded_key.push(expanded_key[i - key_length] ^ expanded_key[i - 4]);
            expanded_key.push(expanded_key[i + 1 - key_length] ^ expanded_key[i - 3]);
            expanded_key.push(expanded_key[i + 2 - key_length] ^ expanded_key[i - 2]);
            expanded_key.push(expanded_key[i + 3 - key_length] ^ expanded_key[i - 1]);
        };
    }

    Ok(expanded_key)
}

fn gmul(a: u8, b: u8) -> u8 {
    let mut a = a;
    let mut b = b;
    let mut product = 0;

    for _ in 0..8 {
        if b & 0x01 == 0x01 {
            product ^= a;
        }

        let high_bit_set = a & 0x80 == 0x80;

        a <<= 1;

        if high_bit_set {
            a ^= 0x1b;
        }

        b >>= 1;
    }

    product
}

fn inverse_mix_column(column: &mut [u8]) {
    let copy = column.to_vec();

    column[0] = gmul(copy[0], 14) ^ gmul(copy[3], 9) ^ gmul(copy[2], 13) ^ gmul(copy[1], 11);
    column[1] = gmul(copy[1], 14) ^ gmul(copy[0], 9) ^ gmul(copy[3], 13) ^ gmul(copy[2], 11);
    column[2] = gmul(copy[2], 14) ^ gmul(copy[1], 9) ^ gmul(copy[0], 13) ^ gmul(copy[3], 11);
    column[3] = gmul(copy[3], 14) ^ gmul(copy[2], 9) ^ gmul(copy[1], 13) ^ gmul(copy[0], 11);
}

fn inverse_mix_columns(state: &mut [u8]) {
    for i in (0..16).step_by(4) {
        inverse_mix_column(&mut state[i..(i + 4)]);
    }
}

fn inverse_shift_rows(state: &mut [u8]) {
    let temp = state[1];
    state[1] = state[13];
    state[13] = state[9];
    state[9] = state[5];
    state[5] = temp;

    state.swap(2, 10);
    state.swap(6, 14);

    let temp = state[3];
    state[3] = state[7];
    state[7] = state[11];
    state[11] = state[15];
    state[15] = temp;
}

fn mix_column(column: &mut [u8]) {
    let copy = column.to_vec();
    let mut doubles = [0, 0, 0, 0];

    for i in 0..4 {
        let h = if column[i] & 0x80 == 0x80 { 0x1b } else { 0x00 };

        doubles[i] = (column[i] << 1) ^ h;
    }

    // This is slightly optimized from the gmul version and is equivalent to:
    // column[0] = gmul(copy[0], 2) ^ gmul(copy[3], 1) ^ gmul(copy[2], 1) ^ gmul(copy[1], 3);
    // column[1] = gmul(copy[1], 2) ^ gmul(copy[0], 1) ^ gmul(copy[3], 1) ^ gmul(copy[2], 3);
    // column[2] = gmul(copy[2], 2) ^ gmul(copy[1], 1) ^ gmul(copy[0], 1) ^ gmul(copy[3], 3);
    // column[3] = gmul(copy[3], 2) ^ gmul(copy[2], 1) ^ gmul(copy[1], 1) ^ gmul(copy[0], 3);
    column[0] = doubles[0] ^ copy[3] ^ copy[2] ^ doubles[1] ^ copy[1];
    column[1] = doubles[1] ^ copy[0] ^ copy[3] ^ doubles[2] ^ copy[2];
    column[2] = doubles[2] ^ copy[1] ^ copy[0] ^ doubles[3] ^ copy[3];
    column[3] = doubles[3] ^ copy[2] ^ copy[1] ^ doubles[0] ^ copy[0];
}

fn mix_columns(state: &mut [u8]) {
    for i in (0..16).step_by(4) {
        mix_column(&mut state[i..(i + 4)]);
    }
}

fn shift_rows(state: &mut [u8]) {
    let temp = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = temp;

    state.swap(2, 10);
    state.swap(6, 14);

    let temp = state[3];
    state[3] = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = temp;
}

fn sub_bytes(state: &mut [u8], s_box: &[u8]) {
    for i in 0..16 {
        state[i] = s_box[state[i] as usize];
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn add_round_key_works() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.

        #[rustfmt::skip]
        let mut state = vec![
            0x3e, 0xc0, 0x8b, 0x72, 0xdd, 0x6a, 0xec, 0x9b, 0x52, 0xaa, 0x86, 0xac, 0xda, 0xc7, 0xeb, 0xb1,
        ];
        #[rustfmt::skip]
        let key_schedule = vec![
            0xb1, 0xf6, 0x90, 0x6f, 0x7e, 0x4a, 0xcf, 0x08, 0x6d, 0xf7, 0x51, 0x20, 0xb6, 0x90, 0xd0, 0x7e,
            0x1d, 0x12, 0x51, 0xef, 0x59, 0x76, 0x6a, 0xde, 0x84, 0x18, 0x1c, 0x3d, 0x4b, 0x7d, 0x1b, 0x13,
            0x53, 0x36, 0xf3, 0x15, 0xb4, 0xcb, 0x2a, 0x34, 0xe7, 0x3d, 0x46, 0xc1, 0x74, 0xc2, 0x37, 0x60,
            0xf8, 0x87, 0x4a, 0x83, 0x99, 0x29, 0xd2, 0x2c, 0x0a, 0x2f, 0x70, 0xc8, 0x37, 0x1d, 0x2e, 0x09,
            0x98, 0xa4, 0xf3, 0xc1, 0x95, 0x2d, 0x7c, 0xcb, 0xca, 0x8d, 0x5a, 0xef, 0xe7, 0xfd, 0xfe, 0x1d,
            0xda, 0xb1, 0x1d, 0xb7, 0xa1, 0x67, 0x27, 0xdb, 0x00, 0x82, 0xcd, 0xf2, 0x31, 0x7b, 0x8c, 0x3b,
            0x04, 0x54, 0x58, 0x3a, 0xd1, 0x58, 0xd3, 0x5c, 0x61, 0x84, 0x3f, 0x91, 0xc3, 0xe5, 0xa8, 0x8b,

            // Round key 7.
            0xf8, 0x0d, 0x37, 0xe6, 0x50, 0xe2, 0x34, 0x87, 0x44, 0xfb, 0x2d, 0x23, 0xf5, 0x69, 0x58, 0x28,

            0x83, 0xa4, 0x46, 0x38, 0x1a, 0xb0, 0x4e, 0x99, 0xb9, 0x37, 0xb5, 0x1b, 0x49, 0x59, 0x9e, 0x26,
            0xcd, 0x08, 0x00, 0xd3, 0x2d, 0x3b, 0x84, 0xa8, 0x18, 0x9e, 0x9f, 0xe5, 0x68, 0x6b, 0x68, 0xa8,
            0xcf, 0x38, 0x31, 0xbb, 0x18, 0xd8, 0xe1, 0x82, 0x3f, 0x02, 0x8a, 0xaa, 0x0c, 0xe8, 0x6e, 0xfb,
        ];

        add_round_key(&mut state, &key_schedule, 7);

        #[rustfmt::skip]
        assert_eq!(state, vec![
            0xc6, 0xcd, 0xbc, 0x94, 0x8d, 0x88, 0xd8, 0x1c, 0x16, 0x51, 0xab, 0x8f, 0x2f, 0xae, 0xb3, 0x99,
        ]);
    }

    #[test]
    fn decrypt_block_128_1() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0x70, 0xbd, 0xc9, 0x0f, 0xa7, 0xd2, 0xee, 0xd2, 0xbf, 0x04, 0xae, 0xfe, 0x67, 0xc6, 0x1e, 0xec,
        ], &vec![
            0x13, 0x5b, 0x42, 0x2a, 0x05, 0x97, 0xf0, 0xfc, 0x1d, 0x3c, 0x67, 0x21, 0xa0, 0x41, 0x05, 0x60,
        ]), Ok(vec![
            0x04, 0xbb, 0x67, 0xb0, 0x62, 0xaa, 0xe5, 0xa9, 0x64, 0x9c, 0x9c, 0x68, 0x7f, 0xd7, 0xe8, 0x8f,
        ]));
    }

    #[test]
    fn decrypt_block_128_2() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0xe8, 0x2b, 0x5a, 0xd0, 0xad, 0xe9, 0xf6, 0x06, 0x31, 0xc0, 0xa8, 0xa2, 0x1d, 0xdf, 0xbf, 0x8c,
        ], &vec![
            0x01, 0xb0, 0xc5, 0x2d, 0x76, 0xbb, 0x11, 0xc0, 0x33, 0x5a, 0x92, 0x19, 0xe7, 0xfd, 0xba, 0x24,
        ]), Ok(vec![
            0x79, 0xed, 0x9f, 0xdf, 0x94, 0xe7, 0x58, 0xb0, 0xb4, 0x7e, 0x69, 0xc7, 0x6b, 0x5c, 0xc0, 0xf3,
        ]));
    }

    #[test]
    fn decrypt_block_128_3() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0x40, 0x01, 0xc2, 0xdb, 0x8c, 0x70, 0x50, 0xb5, 0x01, 0xa6, 0x12, 0xc8, 0x2e, 0xe7, 0xb4, 0xe0,
        ], &vec![
            0x72, 0x65, 0xbd, 0xf0, 0xe2, 0x6c, 0xfb, 0x3f, 0xfe, 0x12, 0x02, 0x69, 0xa1, 0xc5, 0xc2, 0xc9,
        ]), Ok(vec![
            0xa2, 0xdb, 0x8a, 0x9b, 0x0a, 0x31, 0x58, 0x59, 0x93, 0xbe, 0xc6, 0x59, 0x67, 0x06, 0x50, 0x70,
        ]));
    }

    #[test]
    fn decrypt_block_128_4() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0x30, 0x51, 0x6b, 0xf8, 0xf7, 0x78, 0x70, 0x98, 0x82, 0x5b, 0xde, 0x26, 0xad, 0x7d, 0xef, 0x1b,
        ], &vec![
            0xac, 0xd2, 0x14, 0x9e, 0x6c, 0x72, 0x43, 0xdb, 0x11, 0x9e, 0x9f, 0x45, 0x0c, 0xe7, 0x93, 0xae,
        ]), Ok(vec![
            0x52, 0xf8, 0x35, 0xfe, 0xc6, 0xdd, 0xc1, 0xde, 0x3a, 0xbc, 0x9c, 0x1f, 0x91, 0x92, 0xf0, 0xcc,
        ]));
    }

    #[test]
    fn decrypt_block_192_1() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0x2e, 0x2b, 0x66, 0xab, 0x42, 0xab, 0x80, 0x2a, 0xec, 0x63, 0xf8, 0x58, 0xea, 0x8a, 0x6d, 0x7d,
        ], &vec![
            0x13, 0x5b, 0x42, 0x2a, 0x05, 0x97, 0xf0, 0xfc, 0x1d, 0x3c, 0x67, 0x21, 0xa0, 0x41, 0x05, 0x60,
            0xc0, 0xd0, 0x89, 0xbd, 0x77, 0x2b, 0x6c, 0x82,
        ]), Ok(vec![
            0x04, 0xbb, 0x67, 0xb0, 0x62, 0xaa, 0xe5, 0xa9, 0x64, 0x9c, 0x9c, 0x68, 0x7f, 0xd7, 0xe8, 0x8f,
        ]));
    }

    #[test]
    fn decrypt_block_192_2() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0xd0, 0x89, 0xfd, 0x28, 0x8a, 0x1a, 0x17, 0x92, 0xc4, 0xde, 0xbe, 0xf2, 0x93, 0x6f, 0xf0, 0x56,
        ], &vec![
            0x01, 0xb0, 0xc5, 0x2d, 0x76, 0xbb, 0x11, 0xc0, 0x33, 0x5a, 0x92, 0x19, 0xe7, 0xfd, 0xba, 0x24,
            0x7c, 0xc3, 0x91, 0xaf, 0x81, 0x97, 0x57, 0x54,
        ]), Ok(vec![
            0x79, 0xed, 0x9f, 0xdf, 0x94, 0xe7, 0x58, 0xb0, 0xb4, 0x7e, 0x69, 0xc7, 0x6b, 0x5c, 0xc0, 0xf3,
        ]));
    }

    #[test]
    fn decrypt_block_192_3() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0x68, 0xb6, 0x8f, 0x99, 0x7c, 0x8b, 0x4d, 0x22, 0x2e, 0x8e, 0xa5, 0x0c, 0x9c, 0xf6, 0x8e, 0x9d,
        ], &vec![
            0x72, 0x65, 0xbd, 0xf0, 0xe2, 0x6c, 0xfb, 0x3f, 0xfe, 0x12, 0x02, 0x69, 0xa1, 0xc5, 0xc2, 0xc9,
            0xe6, 0xed, 0xc7, 0xba, 0x1d, 0xe5, 0x35, 0x3d,
        ]), Ok(vec![
            0xa2, 0xdb, 0x8a, 0x9b, 0x0a, 0x31, 0x58, 0x59, 0x93, 0xbe, 0xc6, 0x59, 0x67, 0x06, 0x50, 0x70,
        ]));
    }

    #[test]
    fn decrypt_block_192_4() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0x50, 0xe5, 0xf2, 0x12, 0xc9, 0xf2, 0xbd, 0xa7, 0x0c, 0xf1, 0xcb, 0x2b, 0xa7, 0x9e, 0x89, 0x02,
        ], &vec![
            0xac, 0xd2, 0x14, 0x9e, 0x6c, 0x72, 0x43, 0xdb, 0x11, 0x9e, 0x9f, 0x45, 0x0c, 0xe7, 0x93, 0xae,
            0x3b, 0x06, 0xe2, 0x86, 0x5a, 0xdd, 0xb6, 0xbf,
        ]), Ok(vec![
            0x52, 0xf8, 0x35, 0xfe, 0xc6, 0xdd, 0xc1, 0xde, 0x3a, 0xbc, 0x9c, 0x1f, 0x91, 0x92, 0xf0, 0xcc,
        ]));
    }

    #[test]
    fn decrypt_block_256_1() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0x51, 0x9b, 0xd8, 0x84, 0x06, 0x3a, 0x07, 0x2d, 0xcb, 0x90, 0xb9, 0x7a, 0x5a, 0x70, 0x84, 0x0d,
        ], &vec![
            0x13, 0x5b, 0x42, 0x2a, 0x05, 0x97, 0xf0, 0xfc, 0x1d, 0x3c, 0x67, 0x21, 0xa0, 0x41, 0x05, 0x60,
            0xc0, 0xd0, 0x89, 0xbd, 0x77, 0x2b, 0x6c, 0x82, 0x63, 0x82, 0x73, 0xe3, 0x18, 0xc3, 0x4e, 0x60,
        ]), Ok(vec![
            0x04, 0xbb, 0x67, 0xb0, 0x62, 0xaa, 0xe5, 0xa9, 0x64, 0x9c, 0x9c, 0x68, 0x7f, 0xd7, 0xe8, 0x8f,
        ]));
    }

    #[test]
    fn decrypt_block_256_2() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0xd7, 0x15, 0xd7, 0xa5, 0xb8, 0x38, 0xe5, 0x5f, 0x40, 0x85, 0xf1, 0xc2, 0x1a, 0xb5, 0x90, 0x03,
        ], &vec![
            0x01, 0xb0, 0xc5, 0x2d, 0x76, 0xbb, 0x11, 0xc0, 0x33, 0x5a, 0x92, 0x19, 0xe7, 0xfd, 0xba, 0x24,
            0x7c, 0xc3, 0x91, 0xaf, 0x81, 0x97, 0x57, 0x54, 0x51, 0x08, 0x10, 0x1b, 0x9c, 0xcb, 0xa6, 0x6e,
        ]), Ok(vec![
            0x79, 0xed, 0x9f, 0xdf, 0x94, 0xe7, 0x58, 0xb0, 0xb4, 0x7e, 0x69, 0xc7, 0x6b, 0x5c, 0xc0, 0xf3,
        ]));
    }

    #[test]
    fn decrypt_block_256_3() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0x54, 0x56, 0x2a, 0x93, 0xf2, 0x54, 0x43, 0x63, 0xfa, 0x8a, 0x11, 0x39, 0x86, 0x17, 0x85, 0x74,
        ], &vec![
            0x72, 0x65, 0xbd, 0xf0, 0xe2, 0x6c, 0xfb, 0x3f, 0xfe, 0x12, 0x02, 0x69, 0xa1, 0xc5, 0xc2, 0xc9,
            0xe6, 0xed, 0xc7, 0xba, 0x1d, 0xe5, 0x35, 0x3d, 0xac, 0xab, 0x05, 0x04, 0x62, 0x0b, 0x11, 0x6a,
        ]), Ok(vec![
            0xa2, 0xdb, 0x8a, 0x9b, 0x0a, 0x31, 0x58, 0x59, 0x93, 0xbe, 0xc6, 0x59, 0x67, 0x06, 0x50, 0x70,
        ]));
    }

    #[test]
    fn decrypt_block_256_4() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(decrypt_block(&vec![
            0x7b, 0x57, 0x79, 0x96, 0xb6, 0x9e, 0xa2, 0x28, 0x8f, 0xb0, 0x58, 0x6a, 0x67, 0xeb, 0x80, 0x39,
        ], &vec![
            0xac, 0xd2, 0x14, 0x9e, 0x6c, 0x72, 0x43, 0xdb, 0x11, 0x9e, 0x9f, 0x45, 0x0c, 0xe7, 0x93, 0xae,
            0x3b, 0x06, 0xe2, 0x86, 0x5a, 0xdd, 0xb6, 0xbf, 0xc4, 0xcd, 0x9d, 0x7f, 0x43, 0x02, 0x58, 0x4c,
        ]), Ok(vec![
            0x52, 0xf8, 0x35, 0xfe, 0xc6, 0xdd, 0xc1, 0xde, 0x3a, 0xbc, 0x9c, 0x1f, 0x91, 0x92, 0xf0, 0xcc,
        ]));
    }

    #[test]
    fn encrypt_block_128_1() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0x04, 0xbb, 0x67, 0xb0, 0x62, 0xaa, 0xe5, 0xa9, 0x64, 0x9c, 0x9c, 0x68, 0x7f, 0xd7, 0xe8, 0x8f,
        ], &vec![
            0x13, 0x5b, 0x42, 0x2a, 0x05, 0x97, 0xf0, 0xfc, 0x1d, 0x3c, 0x67, 0x21, 0xa0, 0x41, 0x05, 0x60,
        ]), Ok(vec![
            0x70, 0xbd, 0xc9, 0x0f, 0xa7, 0xd2, 0xee, 0xd2, 0xbf, 0x04, 0xae, 0xfe, 0x67, 0xc6, 0x1e, 0xec,
        ]));
    }

    #[test]
    fn encrypt_block_128_2() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0x79, 0xed, 0x9f, 0xdf, 0x94, 0xe7, 0x58, 0xb0, 0xb4, 0x7e, 0x69, 0xc7, 0x6b, 0x5c, 0xc0, 0xf3,
        ], &vec![
            0x01, 0xb0, 0xc5, 0x2d, 0x76, 0xbb, 0x11, 0xc0, 0x33, 0x5a, 0x92, 0x19, 0xe7, 0xfd, 0xba, 0x24,
        ]), Ok(vec![
            0xe8, 0x2b, 0x5a, 0xd0, 0xad, 0xe9, 0xf6, 0x06, 0x31, 0xc0, 0xa8, 0xa2, 0x1d, 0xdf, 0xbf, 0x8c,
        ]));
    }

    #[test]
    fn encrypt_block_128_3() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0xa2, 0xdb, 0x8a, 0x9b, 0x0a, 0x31, 0x58, 0x59, 0x93, 0xbe, 0xc6, 0x59, 0x67, 0x06, 0x50, 0x70,
        ], &vec![
            0x72, 0x65, 0xbd, 0xf0, 0xe2, 0x6c, 0xfb, 0x3f, 0xfe, 0x12, 0x02, 0x69, 0xa1, 0xc5, 0xc2, 0xc9,
        ]), Ok(vec![
            0x40, 0x01, 0xc2, 0xdb, 0x8c, 0x70, 0x50, 0xb5, 0x01, 0xa6, 0x12, 0xc8, 0x2e, 0xe7, 0xb4, 0xe0,
        ]));
    }

    #[test]
    fn encrypt_block_128_4() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0x52, 0xf8, 0x35, 0xfe, 0xc6, 0xdd, 0xc1, 0xde, 0x3a, 0xbc, 0x9c, 0x1f, 0x91, 0x92, 0xf0, 0xcc,
        ], &vec![
            0xac, 0xd2, 0x14, 0x9e, 0x6c, 0x72, 0x43, 0xdb, 0x11, 0x9e, 0x9f, 0x45, 0x0c, 0xe7, 0x93, 0xae,
        ]), Ok(vec![
            0x30, 0x51, 0x6b, 0xf8, 0xf7, 0x78, 0x70, 0x98, 0x82, 0x5b, 0xde, 0x26, 0xad, 0x7d, 0xef, 0x1b,
        ]));
    }

    #[test]
    fn encrypt_block_192_1() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0x04, 0xbb, 0x67, 0xb0, 0x62, 0xaa, 0xe5, 0xa9, 0x64, 0x9c, 0x9c, 0x68, 0x7f, 0xd7, 0xe8, 0x8f,
        ], &vec![
            0x13, 0x5b, 0x42, 0x2a, 0x05, 0x97, 0xf0, 0xfc, 0x1d, 0x3c, 0x67, 0x21, 0xa0, 0x41, 0x05, 0x60,
            0xc0, 0xd0, 0x89, 0xbd, 0x77, 0x2b, 0x6c, 0x82,
        ]), Ok(vec![
            0x2e, 0x2b, 0x66, 0xab, 0x42, 0xab, 0x80, 0x2a, 0xec, 0x63, 0xf8, 0x58, 0xea, 0x8a, 0x6d, 0x7d,
        ]));
    }

    #[test]
    fn encrypt_block_192_2() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0x79, 0xed, 0x9f, 0xdf, 0x94, 0xe7, 0x58, 0xb0, 0xb4, 0x7e, 0x69, 0xc7, 0x6b, 0x5c, 0xc0, 0xf3,
        ], &vec![
            0x01, 0xb0, 0xc5, 0x2d, 0x76, 0xbb, 0x11, 0xc0, 0x33, 0x5a, 0x92, 0x19, 0xe7, 0xfd, 0xba, 0x24,
            0x7c, 0xc3, 0x91, 0xaf, 0x81, 0x97, 0x57, 0x54,
        ]), Ok(vec![
            0xd0, 0x89, 0xfd, 0x28, 0x8a, 0x1a, 0x17, 0x92, 0xc4, 0xde, 0xbe, 0xf2, 0x93, 0x6f, 0xf0, 0x56,
        ]));
    }

    #[test]
    fn encrypt_block_192_3() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0xa2, 0xdb, 0x8a, 0x9b, 0x0a, 0x31, 0x58, 0x59, 0x93, 0xbe, 0xc6, 0x59, 0x67, 0x06, 0x50, 0x70,
        ], &vec![
            0x72, 0x65, 0xbd, 0xf0, 0xe2, 0x6c, 0xfb, 0x3f, 0xfe, 0x12, 0x02, 0x69, 0xa1, 0xc5, 0xc2, 0xc9,
            0xe6, 0xed, 0xc7, 0xba, 0x1d, 0xe5, 0x35, 0x3d,
        ]), Ok(vec![
            0x68, 0xb6, 0x8f, 0x99, 0x7c, 0x8b, 0x4d, 0x22, 0x2e, 0x8e, 0xa5, 0x0c, 0x9c, 0xf6, 0x8e, 0x9d,
        ]));
    }

    #[test]
    fn encrypt_block_192_4() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0x52, 0xf8, 0x35, 0xfe, 0xc6, 0xdd, 0xc1, 0xde, 0x3a, 0xbc, 0x9c, 0x1f, 0x91, 0x92, 0xf0, 0xcc,
        ], &vec![
            0xac, 0xd2, 0x14, 0x9e, 0x6c, 0x72, 0x43, 0xdb, 0x11, 0x9e, 0x9f, 0x45, 0x0c, 0xe7, 0x93, 0xae,
            0x3b, 0x06, 0xe2, 0x86, 0x5a, 0xdd, 0xb6, 0xbf,
        ]), Ok(vec![
            0x50, 0xe5, 0xf2, 0x12, 0xc9, 0xf2, 0xbd, 0xa7, 0x0c, 0xf1, 0xcb, 0x2b, 0xa7, 0x9e, 0x89, 0x02,
        ]));
    }

    #[test]
    fn encrypt_block_256_1() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0x04, 0xbb, 0x67, 0xb0, 0x62, 0xaa, 0xe5, 0xa9, 0x64, 0x9c, 0x9c, 0x68, 0x7f, 0xd7, 0xe8, 0x8f,
        ], &vec![
            0x13, 0x5b, 0x42, 0x2a, 0x05, 0x97, 0xf0, 0xfc, 0x1d, 0x3c, 0x67, 0x21, 0xa0, 0x41, 0x05, 0x60,
            0xc0, 0xd0, 0x89, 0xbd, 0x77, 0x2b, 0x6c, 0x82, 0x63, 0x82, 0x73, 0xe3, 0x18, 0xc3, 0x4e, 0x60,
        ]), Ok(vec![
            0x51, 0x9b, 0xd8, 0x84, 0x06, 0x3a, 0x07, 0x2d, 0xcb, 0x90, 0xb9, 0x7a, 0x5a, 0x70, 0x84, 0x0d,
        ]));
    }

    #[test]
    fn encrypt_block_256_2() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0x79, 0xed, 0x9f, 0xdf, 0x94, 0xe7, 0x58, 0xb0, 0xb4, 0x7e, 0x69, 0xc7, 0x6b, 0x5c, 0xc0, 0xf3,
        ], &vec![
            0x01, 0xb0, 0xc5, 0x2d, 0x76, 0xbb, 0x11, 0xc0, 0x33, 0x5a, 0x92, 0x19, 0xe7, 0xfd, 0xba, 0x24,
            0x7c, 0xc3, 0x91, 0xaf, 0x81, 0x97, 0x57, 0x54, 0x51, 0x08, 0x10, 0x1b, 0x9c, 0xcb, 0xa6, 0x6e,
        ]), Ok(vec![
            0xd7, 0x15, 0xd7, 0xa5, 0xb8, 0x38, 0xe5, 0x5f, 0x40, 0x85, 0xf1, 0xc2, 0x1a, 0xb5, 0x90, 0x03,
        ]));
    }

    #[test]
    fn encrypt_block_256_3() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0xa2, 0xdb, 0x8a, 0x9b, 0x0a, 0x31, 0x58, 0x59, 0x93, 0xbe, 0xc6, 0x59, 0x67, 0x06, 0x50, 0x70,
        ], &vec![
            0x72, 0x65, 0xbd, 0xf0, 0xe2, 0x6c, 0xfb, 0x3f, 0xfe, 0x12, 0x02, 0x69, 0xa1, 0xc5, 0xc2, 0xc9,
            0xe6, 0xed, 0xc7, 0xba, 0x1d, 0xe5, 0x35, 0x3d, 0xac, 0xab, 0x05, 0x04, 0x62, 0x0b, 0x11, 0x6a,
        ]), Ok(vec![
            0x54, 0x56, 0x2a, 0x93, 0xf2, 0x54, 0x43, 0x63, 0xfa, 0x8a, 0x11, 0x39, 0x86, 0x17, 0x85, 0x74,
        ]));
    }

    #[test]
    fn encrypt_block_256_4() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(encrypt_block(&vec![
            0x52, 0xf8, 0x35, 0xfe, 0xc6, 0xdd, 0xc1, 0xde, 0x3a, 0xbc, 0x9c, 0x1f, 0x91, 0x92, 0xf0, 0xcc,
        ], &vec![
            0xac, 0xd2, 0x14, 0x9e, 0x6c, 0x72, 0x43, 0xdb, 0x11, 0x9e, 0x9f, 0x45, 0x0c, 0xe7, 0x93, 0xae,
            0x3b, 0x06, 0xe2, 0x86, 0x5a, 0xdd, 0xb6, 0xbf, 0xc4, 0xcd, 0x9d, 0x7f, 0x43, 0x02, 0x58, 0x4c,
        ]), Ok(vec![
            0x7b, 0x57, 0x79, 0x96, 0xb6, 0x9e, 0xa2, 0x28, 0x8f, 0xb0, 0x58, 0x6a, 0x67, 0xeb, 0x80, 0x39,
        ]));
    }

    #[test]
    fn get_key_schedule_128_all_00() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn get_key_schedule_128_all_ff() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn get_key_schedule_128_counting() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn get_key_schedule_128_arbitrary() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0x69, 0x20, 0xe2, 0x99, 0xa5, 0x20, 0x2a, 0x6d, 0x65, 0x6e, 0x63, 0x68, 0x69, 0x74, 0x6f, 0x2a,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn get_key_schedule_192_all_00() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn get_key_schedule_192_all_ff() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
            0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn get_key_schedule_192_counting() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
            0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn get_key_schedule_256_all_00() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn get_key_schedule_256_all_ff() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
            0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn get_key_schedule_256_counting() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.
        #[rustfmt::skip]
        assert_eq!(get_key_schedule(&vec![
            0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
            0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f,
        ]).unwrap(), vec![
            ,
        ]);
    }

    #[test]
    fn gmul_1() {
        assert_eq!(gmul(0x12, 0x34), 0x05);
    }

    #[test]
    fn gmul_2() {
        assert_eq!(gmul(0x43, 0x21), 0xfb);
    }

    #[test]
    fn gmul_3() {
        assert_eq!(gmul(0xde, 0xad), 0xf1);
    }

    #[test]
    fn inverse_mix_column_1() {
        let mut column = [0x8e, 0x4d, 0xa1, 0xbc];

        inverse_mix_column(&mut column);

        assert_eq!(column, [0xdb, 0x13, 0x53, 0x45]);
    }

    #[test]
    fn inverse_mix_column_2() {
        let mut column = [0x9f, 0xdc, 0x58, 0x9d];

        inverse_mix_column(&mut column);

        assert_eq!(column, [0xf2, 0x0a, 0x22, 0x5c]);
    }

    #[test]
    fn inverse_mix_column_3() {
        let mut column = [0x01, 0x01, 0x01, 0x01];

        inverse_mix_column(&mut column);

        assert_eq!(column, [0x01, 0x01, 0x01, 0x01]);
    }

    #[test]
    fn inverse_mix_column_4() {
        let mut column = [0xc6, 0xc6, 0xc6, 0xc6];

        inverse_mix_column(&mut column);

        assert_eq!(column, [0xc6, 0xc6, 0xc6, 0xc6]);
    }

    #[test]
    fn inverse_mix_column_5() {
        let mut column = [0xd5, 0xd5, 0xd7, 0xd6];

        inverse_mix_column(&mut column);

        assert_eq!(column, [0xd4, 0xd4, 0xd4, 0xd5]);
    }

    #[test]
    fn inverse_mix_column_6() {
        let mut column = [0x4d, 0x7e, 0xbd, 0xf8];

        inverse_mix_column(&mut column);

        assert_eq!(column, [0x2d, 0x26, 0x31, 0x4c]);
    }

    #[test]
    fn inverse_shift_rows_works() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.

        #[rustfmt::skip]
        let mut state = vec![
            0x00, 0x01, 0x02, 0x03,
            0x04, 0x05, 0x06, 0x07,
            0x08, 0x09, 0x0a, 0x0b,
            0x0c, 0x0d, 0x0e, 0x0f,
        ];

        inverse_shift_rows(&mut state);

        #[rustfmt::skip]
        assert_eq!(state, vec![
            0x00, 0x0d, 0x0a, 0x07,
            0x04, 0x01, 0x0e, 0x0b,
            0x08, 0x05, 0x02, 0x0f,
            0x0c, 0x09, 0x06, 0x03,
        ]);
    }

    #[test]
    fn mix_column_1() {
        let mut column = [0xdb, 0x13, 0x53, 0x45];

        mix_column(&mut column);

        assert_eq!(column, [0x8e, 0x4d, 0xa1, 0xbc]);
    }

    #[test]
    fn mix_column_2() {
        let mut column = [0xf2, 0x0a, 0x22, 0x5c];

        mix_column(&mut column);

        assert_eq!(column, [0x9f, 0xdc, 0x58, 0x9d]);
    }

    #[test]
    fn mix_column_3() {
        let mut column = [0x01, 0x01, 0x01, 0x01];

        mix_column(&mut column);

        assert_eq!(column, [0x01, 0x01, 0x01, 0x01]);
    }

    #[test]
    fn mix_column_4() {
        let mut column = [0xc6, 0xc6, 0xc6, 0xc6];

        mix_column(&mut column);

        assert_eq!(column, [0xc6, 0xc6, 0xc6, 0xc6]);
    }

    #[test]
    fn mix_column_5() {
        let mut column = [0xd4, 0xd4, 0xd4, 0xd5];

        mix_column(&mut column);

        assert_eq!(column, [0xd5, 0xd5, 0xd7, 0xd6]);
    }

    #[test]
    fn mix_column_6() {
        let mut column = [0x2d, 0x26, 0x31, 0x4c];

        mix_column(&mut column);

        assert_eq!(column, [0x4d, 0x7e, 0xbd, 0xf8]);
    }

    #[test]
    fn shift_rows_works() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.

        #[rustfmt::skip]
        let mut state = vec![
            0x00, 0x01, 0x02, 0x03,
            0x04, 0x05, 0x06, 0x07,
            0x08, 0x09, 0x0a, 0x0b,
            0x0c, 0x0d, 0x0e, 0x0f,
        ];

        shift_rows(&mut state);

        #[rustfmt::skip]
        assert_eq!(state, vec![
            0x00, 0x05, 0x0a, 0x0f,
            0x04, 0x09, 0x0e, 0x03,
            0x08, 0x0d, 0x02, 0x07,
            0x0c, 0x01, 0x06, 0x0b,
        ]);
    }

    #[test]
    fn sub_bytes_works() {
        // rustfmt will wrap at 14 bytes, which makes these tables harder to read.

        #[rustfmt::skip]
        let mut state = vec![
            0x98, 0x59, 0x86, 0xc8, 0x0c, 0x12, 0x10, 0xac, 0xba, 0x8f, 0xb8, 0x0b, 0x5e, 0x69, 0x20, 0x75,
        ];

        sub_bytes(&mut state, &INVERSE_S_BOX);

        #[rustfmt::skip]
        assert_eq!(state, vec![
            0xe2, 0x15, 0xdc, 0xb1, 0x81, 0x39, 0x7c, 0xaa, 0xc0, 0x73, 0x9a, 0x9e, 0x9d, 0xe4, 0x54, 0x3f,
        ]);
    }
}
